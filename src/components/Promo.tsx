export default function Promo() {
  return (
    <section className="py-12 px-6 bg-red-50 text-center">
      <h2 className="text-3xl font-semibold mb-4">Promotions</h2>
      <p className="text-lg mb-4">
        🎉 Wave 125s Coversets & Screw Sets available at wholesale rates!
        <br />
        🛍️ Our online shop is coming soon — stay tuned!
      </p>
      <a
        href="https://invite.viber.com/?g2=AQB6VW5%2BjEpi%2BVNDeeMeKCx1%2ByKhzHEzOHAkWi5iYe5aLoRiBC33rfZmXXzKPavI"
        target="_blank"
        rel="noopener noreferrer"
        className="inline-block mt-4 bg-red-600 text-white px-6 py-3 rounded-full hover:bg-red-700"
      >
        Join Our Viber Group
      </a>
    </section>
  );
}
